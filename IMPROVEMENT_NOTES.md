...
### ~~9. 유닛 생산 위치 점유 검사~~ ✅ 완료 (2026-05-06)

- 위치: `scripts/main.js` → `spawnUnitNear()`, `scripts/entities.js` → `Building.update()`
- **수정 완료.** 아래 내용은 기록 목적으로만 남겨둔다.

변경 내용:

- `spawnUnitNear()` 후보 타일마다 `unit.tileX/tileY`로 점유 여부를 확인해 이미 유닛이 있는 타일은 건너뜀.
- 모든 후보가 막혀 있으면 `null` 반환 후 로그 출력.
- `Building.update()` 병영 분기에서 반환값을 확인해 실패 시 gold 환급 및 3초 후 재시도.
...
| 2026-05-06 | 유닛 생산 위치 점유 검사 (tileX/tileY 점유 확인, 실패 시 gold 환급·3초 재시도) |
